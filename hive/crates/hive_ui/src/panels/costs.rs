...
use crate::workspace::{CostsClearHistory, CostsExportCsv, CostsResetToday};
...
impl CostData {
    ...
    pub fn from_tracker(tracker: &CostTracker) -> Self {
        ...
        let mut by_model: Vec<ModelCostEntry> = model_map
            .into_iter()
            .map(|(model_id, (requests, input_tokens, output_tokens, cost))| ModelCostEntry {
                model_id,
                requests,
                input_tokens,
                output_tokens,
...
impl CostsPanel {
    ...
    fn action_buttons(theme: &HiveTheme) -> impl IntoElement {
        div()
            ...
            .flex_row()
            .gap(theme.space_3)
            .child(Self::action_btn("Export CSV", "costs-export-csv", theme.accent_cyan, theme)
                .on_mouse_down(MouseButton::Left, |_event, _window, cx| {
                    cx.dispatch_action(&CostsExportCsv);
                }))
            .child(Self::action_btn("Reset Today", "costs-reset-today", theme.accent_yellow, theme)
                .on_mouse_down(MouseButton::Left, |_event, _window, cx| {
                    cx.dispatch_action(&CostsResetToday);
                }))
            .child(div().flex_1()) // spacer pushes Clear to the right
            .child(Self::action_btn("Clear History", "costs-clear-history", theme.accent_red, theme)
                .on_mouse_down(MouseButton::Left, |_event, _window, cx| {
