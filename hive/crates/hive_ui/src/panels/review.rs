...
use crate::workspace::{ReviewCommit, ReviewDiscardAll, ReviewStageAll, ReviewUnstageAll};
...
impl ReviewData {
    ...
    pub fn from_git(repo_path: &Path) -> Self {
        let git = match GitService::open(repo_path) {
            Ok(g) => g,
            Err(e) => {
                warn!("ReviewData: could not open git repo at {}: {e}", repo_path.display());
                return Self::empty();
            }
        };
        ...
        let (files, modified_count, staged_count, untracked_count) =
            Self::load_file_statuses(&git);
    ...
    fn load_file_statuses(
        git: &GitService,
    ) -> (Vec<ReviewFileEntry>, usize, usize, usize) {
    ...
    pub fn parse_diff(raw: &str) -> Vec<DiffLine> {
        ...
        for text in raw.lines() {
            ...
            } else if text.starts_with("diff ") || text.starts_with("index ")
...
impl ReviewPanel {
    ...
    fn file_staged_cell(entry: &ReviewFileEntry, theme: &HiveTheme) -> Div {
        div()
            ...
            .child(if entry.is_staged { "\u{2713} Yes" } else { "No" })
    }
    ...
    fn render_diff_line(line: &DiffLine, theme: &HiveTheme) -> impl IntoElement {
        let (bg, text_color, prefix) = match line.kind {
            DiffLineKind::Addition => (
                hsla(120.0 / 360.0, 0.4, 0.12, 0.4),
                theme.accent_green,
                "+",
            ),
            DiffLineKind::Deletion => (
                hsla(0.0, 0.4, 0.12, 0.4),
                theme.accent_red,
                "-",
            ),
            DiffLineKind::Hunk => (
                hsla(220.0 / 360.0, 0.3, 0.15, 0.3),
                theme.accent_cyan,
                "@",
            ),
    ...
    fn comments_card(data: &ReviewData, theme: &HiveTheme) -> impl IntoElement {
        ...
        if data.comments.is_empty() {
            container = container.child(Self::centered_message(
                "No comments yet. Click on a diff line to add a comment.", theme));
        } else {
    ...
    fn review_btn(label: &str, color: Hsla, is_active: bool, theme: &HiveTheme) -> impl IntoElement {
    ...
    fn git_actions(theme: &HiveTheme) -> impl IntoElement {
        div()
            ...
            .child(
                Self::action_btn("review-unstage-all", "Unstage All", theme.accent_yellow, theme)
                    .on_mouse_down(MouseButton::Left, |_event, window, cx| {
