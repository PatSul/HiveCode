...
use crate::workspace::KanbanAddTask;
...
impl KanbanData {
    ...
    pub fn sample() -> Self {
        ...
        data.add_task(0, "Set up CI pipeline", "Configure GitHub Actions for build, lint, and test on every PR.", Priority::High);
        data.add_task(0, "Write auth unit tests", "Cover login, token refresh, and session expiry edge cases.", Priority::Medium);
        data.add_task(0, "Add rate-limit middleware", "Per-IP sliding window, 60 req/min default, configurable.", Priority::Low);

        data.add_task(1, "Implement chat streaming", "Wire SSE endpoint to the AI provider and render tokens incrementally.", Priority::Critical);
        data.add_task(1, "Refactor model router", "Extract fallback logic into its own module, add retry budgets.", Priority::Medium);

        data.add_task(2, "Review security gateway", "Audit command allowlist and URL validation against OWASP checklist.", Priority::High);

        data.add_task(3, "Project scaffold", "Workspace layout, Cargo.toml, initial crate structure.", Priority::High);
        data.add_task(3, "Theme system", "Design tokens, dark palette, HiveTheme struct with all colors.", Priority::Medium);
        data.add_task(3, "Sidebar navigation", "Icon buttons, active state highlight, panel switching.", Priority::Medium);
...
impl KanbanPanel {
    ...
    fn toolbar(theme: &HiveTheme) -> impl IntoElement {
        div()
            ...
            .child(Self::toolbar_btn("Delete Selected", theme.accent_red, theme))
            // Add task
            .child(
                div()
                    .id("kanban-add-task")
