...
use crate::workspace::SkillsRefresh;
...
fn render_tab_bar(active: &SkillsTab, theme: &HiveTheme) -> AnyElement {
    div()
        ...
        .border_b_1()
        .border_color(theme.border)
        .child(tab_pill("Installed", *active == SkillsTab::Installed, theme))
        .child(tab_pill("Directory", *active == SkillsTab::Directory, theme))
...
fn render_create_form(
    ...
    div()
        ...
        .child(form_field_display("Version", &draft.version, "0.1.0", theme))
...
fn instructions_field(draft: &CreateSkillDraft, theme: &HiveTheme) -> Div {
    let (text_color, content) = if draft.instructions.is_empty() {
        (theme.text_muted, "You are a helpful assistant that... (write detailed instructions for the AI)".to_string())
    } else {
...
fn toggle_switch(enabled: bool, skill_id: &str, theme: &HiveTheme) -> AnyElement {
    ...
    div()
        ...
        .child(toggle_track(track_bg, knob_bg, border_color, enabled, theme))
        .child(
            div()
                .text_size(theme.font_size_sm)
